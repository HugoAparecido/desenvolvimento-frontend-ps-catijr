...
export function RightClickMenuItem({ item }: RightClickMenuItem) {
    ...
    return (
        <div className="w-54 h-7.5 relative"
        ...
        >
            <div className="w-auto h-auto flex justify-between items-center rotate-0.5 gap-2.5 p-2 bg-transparent">
                <div className="gap-2 flex justify-center items-center">
                    ...
                    <span className="font-poppins font-medium text-xs text-text-subdued">
                        {item.text}
                    </span>
                </div>
                {hasDropdown && (
                    <img src="/public/action/dropdown_arrow.svg" alt="Dropdown arrow" className="h-3.5" />
                )}
            </div>
            {item.hasDivider && (
                <div className="w-full border border-0.5 border-essential-subdued" />
            )}
