...
export function ArtistPopularSongs({ artistId }: ArtistPopularSongsProps) {
    const { songs, handleViewMoreClick, hasMore } = useArtistSong(artistId);
    ...
    return (
        <div className="flex flex-col gap-2.5">
            ...
            <LinkButton text={!hasMore ? "Mostrar tudo" : "Mostrar menos"} onClick={handleViewMoreClick} variant="default_subdued_10" />
        </div>
    )
}